...
def func_worker(axon_path, dest, distance_threshold=5.0):
    ...
    try:
        swc = readSWC(axon_path)
        axon = swc.loc[swc.type == 2, ["x", "y", "z"]].astype(float)
        if axon.empty:
        ...
        # The tile/cKDTree stage only discovers candidates.  Give it a tiny
        # numerical margin so a true match is never discarded before the exact
        # scipy KDTree query below.  The final acceptance threshold remains
        # exactly distance_threshold, matching the original implementation.
        candidate_radius = (
            distance_threshold
        ...
        for dendrite_code, rows in by_dendrite.items():
            ...
            # KDTree returns index == len(den) and distance == inf when the
            # exact query has no neighbour inside distance_upper_bound.  This
            # can occur for points extremely close to the threshold even when
            # the spatial cKDTree candidate stage included them.  The original
            # implementation ignores these entries through ``dd <= threshold``;
            # indexing den.iloc directly would instead raise IndexError and
            # abort the rest of this axon.
            valid = (
...
if __name__ == "__main__":
    ...
    if FULL:
        # use complete neuron axon morphology data
        axon_src = ['./data/1um_ION_Hipp_full_SWC_CCFv3/ION_Hipp_full_SWC_CCFv3',
        ...
        dest = './results/results_pac_optimized_full'
        log_path = './log/log_pac_optimized_full'
        index_path = './cache/spatial_dendrite_tiles_full_pac'
    else:
        # use example neuron axon morphology data
        axon_src = ['./data/axon_example']

        # use example neuron dendrite morphology data
        src2 = './data/dendrite_example'
        dest = './results/results_pac_optimized'
        log_path = './log/log_pac_optimized'
        index_path = './cache/spatial_dendrite_tiles_pac'
    ...
    cores = min(32, multiprocessing.cpu_count())  # int(multiprocessing.cpu_count() * 0.8)
    print("FULL: ", FULL)
    print("cores: ", cores)
    ...
    if pending_axons:
        ...
        for task in tasks:
            try:
                result = task.get(timeout=36000)  # 10 hour timeout
