...
def _worker(args):
    ...
    try:
        ...
        tmp_den = tmp_den_.loc[tmp_r['den_id'], ['x', 'y', 'z']].copy()
        dis_dict = tmp_r.set_index('den_id')['dis'].to_dict()
        tmp_den['dis'] = tmp_den.index.map(dis_dict)
...
if __name__ == "__main__":
    ...
    add_region_info(meta_file=f'./output/ppss_table_pac.csv',
                     save_path=f'./output/ppss_from_pacs.csv')

    # get ppss based on axon bouton sites            
    get_ppss_table_multi(n_jobs=20, dest='./results/results_pb',
    ...
    # get_ppss_table_multi(n_jobs=20, dest='./results/results_pb_with_connectivity',
    #                      save_path='./output/ppss_table_pb_with_connectivity.csv')

    # add_region_info(meta_file='./output/ppss_table_pb_with_connectivity.csv',
    #                 save_path='./output/ppss_from_boutons_with_connectivity.csv')
